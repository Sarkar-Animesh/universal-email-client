...
import { GoogleIcon, LockIcon, AlertIcon, RefreshIcon } from "@/components/Icons";
...
function CallbackInner() {
  ...
  async function doExchange() {
    ...
    const stateExpected = sessionStorage.getItem("gmail_oauth_state");
    const verifier = sessionStorage.getItem("gmail_oauth_verifier");
    const redirect = sessionStorage.getItem("gmail_oauth_redirect");
    ...
    if (!key) {
      // Key wiped during the Google redirect (full-page navigation clears JS
      // memory). Show an inline unlock form so the OAuth code in the URL is
      // preserved — we complete the exchange after unlocking.
    ...
    try {
      const { tokens, account } = await auth.gmailCallback({
        code,
  ...
  async function handleUnlock(e: React.FormEvent) {
    ...
    // Key is now in memory — complete the exchange without navigating.
    await doExchange();
  }
